...
export function Marquee() {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden border-y border-[#1f4a80] bg-navy py-3.5 text-white [mask-image:linear-gradient(90deg,transparent,#000_6%,#000_94%,transparent)]"
    >
      <div className="marquee-track flex w-max items-center gap-0 motion-reduce:animate-none">
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              <Icon className="w-4 shrink-0 text-ice" strokeWidth={2} />
              <span className="whitespace-nowrap text-[0.92rem] font-semibold uppercase tracking-[0.06em] text-[#d6e8fb]">
                {item.text}
              </span>
              <span className="ml-1 h-1 w-1 rounded-full bg-ice/40" />
            </div>
          )
        })}
      </div>
